...
public class C06E09_FillOutFlattenAndMergeForms {
    ...
    public static final String SRC2 = "src/main/resources/pdf/state2.pdf";
//    public static final String SRC2 = "src/main/resources/pdf/subscribe.pdf";
    ...
    public void createPdf(String dest1, String dest2) throws IOException {
        ...
        while ((line = bufferedReader.readLine()) != null && i ==1) {
            ...
            fields.get("name").setValue("Teste");
...
//            PdfDocument sourcePdfDocument2 = new PdfDocument(new PdfReader(SRC2));
            PdfDocument sourcePdfDocument2 = new PdfDocument(new PdfReader(SRC2), new PdfWriter(baos));

            PdfAcroForm form2 = PdfAcroForm.getAcroForm(sourcePdfDocument2, true);
            Map<String, PdfFormField> fields2 = form2.getFormFields();
            fields2.get("name").setValue("Felipe Martins");
            form2.flattenFields();

            //Copy pages
            System.out.println(sourcePdfDocument.getNumberOfPages());
            System.out.println(destPdfDocument.getNumberOfPages());
            sourcePdfDocument.copyPagesTo(1, sourcePdfDocument.getNumberOfPages(), destPdfDocument, null);
            sourcePdfDocument.copyPagesTo(1, sourcePdfDocument2.getNumberOfPages(), destPdfDocument, null);

            sourcePdfDocument.copyPagesTo(1, sourcePdfDocument.getNumberOfPages(), destPdfDocumentSmartMode, null);

            sourcePdfDocument.close();
            sourcePdfDocument2.close();
        }
    ...
    public void createPdf1(String dest1, String dest2) throws IOException {
        PdfDocument destPdfDocument = new PdfDocument(new PdfWriter(dest1));
        //Smart mode
        PdfDocument destPdfDocumentSmartMode = new PdfDocument(new PdfWriter(dest2).setSmartMode(true));
